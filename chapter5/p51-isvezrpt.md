...
- <font color="red">各ステップに該当する詳細な仕様はそのステップ内に書かれており，それらを全て考慮する．</font>
...
なお，ステップ1/3と2/3では処理を`main` 関数内で実装すること．
...
- `main` 関数内で標準出力を行う．出力に関しては, 問題2-1(2-1. 最小値検索【初歩】)の`print_array`関数を参考にせよ．
  ...
  - 比較を行うたびに，比較回数`comp`と最小値`min`を出力し，改行せよ．例の`comp`=10，`min`=1の時に出力形式は以下のとおりとする．
    ```　
	comp⊔=⊔10,⊔min⊔=⊔1↩︎
    ```
