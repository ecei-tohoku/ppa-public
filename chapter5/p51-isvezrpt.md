...
- 可変長配列を用いて配列を確保すること．可変長配列では，以下の例のように，配列を宣言するときの要素数を変数にできる．
    - 例：
    ```
      int main(){
        int n;              // 配列の要素数
...
- 比較回数を数える時に以下を参考にする．
  ...
  ```
  <font color="red">カウンタを条件分岐の中に入れないこと．</font>
...
- 1-５. 配列の要素数と配列データを標準入力する
- 1-2で用意した配列はコメントアウトする．

- 自動採点システムに用意したテストケースでは，配列の要素数がテストケースごとに異なる．このため，配列の要素数をテストケースごとに宣言する必要がある．問題5-1〜5-3では，「可変長配列」を用いて配列の要素数を宣言する．
...
- 比較を行うたびに，比較回数`comp`と最小値`min`を出力し，改行せよ．出力形式は，以下のとおりとする．
  - 例：`comp`=10，`min`=1の時
    ```
	comp⊔=⊔10,⊔min⊔=⊔1↩︎
    ```

- 例：入力が8 3 19 8 16 5 11 2 17 の時，出力は以下のようになる．
  ```
...
- ステップ1/3の1-5における，比較を行うたびに出力する部分（`comp⊔=⊔10,⊔min⊔=⊔1↩︎`）は使用しないので，削除する．

#### 2-2. 配列の先頭の要素を除いた残りの配列データの最小値検索

- 最小値（配列の先頭の要素）を除いた残りの配列データの最小値検索し，残りにデータの前に移動する．
- そのように全てのデータに対する最小値検索を行う．
- これらの処理をループをつかて書く．（ソート全体は２重ループでかける．）

#### 2-3. 2-2を残りの配列に対して繰り返す

- ソート処理全体の比較回数を数え，最後に標準出力する．出力の仕方は以下の通り．
    ```
    comp⊔=⊔28↩︎
    ```
...
- 配列を選択ソートによって並べ替えるvoid型の `sel_sort` 関数を作成すること．ステップ2/3で作成した最小値探索を行う部分を用いて，`sel_sort` 関数を作成
