...
- 3-1. `void print_array(int array[], int size)` 関数を作成し，配列データを標準出力する部分はこの関数の中に入れる，

- 3-2. 2つの要素を入れ替える`swap` 関数の作成する.
  ```
    void swap(int array[], int x, int y) 
  ```

- 3-3. 配列を選択ソートによって並べ替える `void sel_sort(int array[], int N)` 関数を作成する．ステップ2/3で作成した最小値探索を行う部分をこの関数内に移動する．
  この関数の引数`N`は配列内のデータ数である．
  ```
    void sel_sort(int array[], int N)
  ```
  `sel_sort` 関数の中で，ソート処理が終わってから比較回数も標準出力する．出力の仕方は2-4と同じ．
