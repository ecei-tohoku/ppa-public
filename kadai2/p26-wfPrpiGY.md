...
配列の要素に直接アクセスしてはいけない。 代わりに、前の課題で指定した`ppa_extra_h/p2_header.h`の関数を使用。 配列の要素を交換するときは、`func_swap_`を使用。

---
## 2.6.1 木構造と配列
---
木構造は、各要素に1つの親ノードと複数の子ノードがあるデータ構造。根ノードは、親ノードのない唯一の要素。詳細については、[木構造（データ構造）](https://ja.wikipedia.org/wiki/%E6%9C%A8%E6%A7%8B%E9%80%A0_(%E3%83%87%E3%83%BC%E3%82%BF%E6%A7%8B%E9%80%A0))の方。
木構造 の1つのタイプは、完全な二分木である。完全な二分木の要素には、最大2つの子ノードがある。二分木では、すべての階がいっぱい。考えられる唯一の例外は、左から右に塗りつぶされている最下階。
完全な二分木は配列で表すことができる。この表現では、配列の0番目の要素が根。次に、 $ i $ 番目の要素ごとに、その子ノードは $ （2 * i + 1） $ 番目と $ （2 * i + 2） $ 番目の要素で表されて、これらの要素が配列の制限を超えない限り。

![](../images/hp_sort.png) 

2.6.1で、入力配列とその配列内のインデックス $ i $ を受け取り、次のように出力するプログラムを作成。
...
- 関数`main`を実装する必要があります
  - この関数内で、`STDIN`からの入力を読み取る。入力は上記の形式に従う。
  - `func_mallocation_`を使用して入力配列にメモリを割り当てる。
  - 配列に入力からの要素を保存。
...
## 2.6.2 最大ヒープの構築
---
最大ヒープは、特定のタイプの完全な二分木である。最大ヒープでは、親ノードの値はその子ノードの値以上。
完全な二分木を表す長さ $ N $ の配列 $ X $ について考えてみる。 $ X $ を最大ヒープに変換するには、次のアルゴリズムを使用。
...
2.6.2で、入力配列を受け取り、最大ヒープを作成するプログラムを作成。
...
#### 出力
ステップ0の反復ごとに、現在の要素のインデックスと考慮される配列の長さを出力し、配列の結果の状態を含む各要素の交換の行を出力し、配列の木構造表現を出力。最後に、配列の最終状態を出力。
...
ヒント：ステップ1を初めて実行するときは、配列内の要素ごとに`downheap`を呼び出す必要。ステップ1が2回目実行されたときから、配列内の1つの要素だけに対して`downheap`を呼び出す必要。

ヒント：ステップ3では、 $ N $ を1つ減らす。 $ N $ は最大ヒープのサイズを示す。 $ N $ の後の配列の要素（つまり、最大ヒープの外側の配列の要素）はすでにソートされている。

![](../images/hp_sort2.png) 

2.6.3で、入力配列を受け取り、ヒープソートを使用してソートするプログラムを作成。
...
#### 仕様
- 関数`heap_sort`を作成する必要
  - この関数は、入力配列（ $ X $ ）と入力配列の長さ（ $ N $ ）を受け取る。
  - 上記のアルゴリズムに従って、ヒープソートを実装。
  - 2.6.2からの関数`downheap`を使用　（`STDOUT`に出力するダウンヒープ内の部分は不要になったため、削除することを忘れないで）
