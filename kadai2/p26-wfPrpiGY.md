...
完全な二分木は配列で表すことができる。この表現では、配列の0番目の要素が根である。次に、 $ i $ 番目の要素ごとに、その子ノードは $ （2 * i + 1） $ 番目と $ （2 * i + 2） $ 番目の要素で表されて、これらの要素が配列の制限を超えない限り。

![](../images/hp_sort.png) 

2.6.1で、入力配列とその配列内のインデックス $ i $ を受け取り、次のように出力するプログラムを作成すること。
