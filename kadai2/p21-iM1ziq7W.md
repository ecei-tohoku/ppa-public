...
ユークリッドの互除法のアルゴリズムを図１のフローチャートに示す．ここでは`r==0`になるまで繰り返す部分の実装方法として，ループを使う方法と再起的呼び出しを使う方法がある．この問題では，再起的呼び出しにより実装する．
<figure>
<img src="prob2-1-1.jpg" width="300">
<figcaption align = "center"><b>図１ - ユークリッドの互除法アルゴリズムのフローチャート</b></figcaption>
</figure>

図２では，初期入力データ整理の部分と繰り返す部分に分けて表現されている．データ整理のブロックを`main`関数，また，繰り返し処理ブロックを`GCD`関数とする．`GCD`関数の中から自分自身を再起的に呼び出している．`r==0`の条件で再起が終了される．このブロック図に基づいてcプログラムを構築することができる．
<figure>
<img src="prob2-1-2.jpg" width="800">
<figcaption align = "center"><b>図２ - main関数と再起的呼び出し</b></figcaption>
</figure>
...
- `int GCD(int a, int b)`関数を再起的に呼び出すこと．
...
- 以下のプログラムを必ず用いてユークリッドの互除法を実装すること．
...
int main() {
  ...
  //大きい値はいつもaになるようにする
  int tmp;
  if(b ??? a) {                                 //大小関係の正しいのを入れる
    tmp = ???;                                  //一時保存
    a = ???;
    b = ???;
  }

  int ans = GCD(???, ???);                     //大きい値と小さい値の順番にamari関数の引数を渡す
  printf("DONE:::::::::::\n%d\n", ???);        //最大公約数を標準出力に出す
...
int GCD(int a, int b) {
  ...
  if(r == 0) {
    ...
    return GCD(???, ???);                      // 自分自身を呼び出す．本問題の説明文を参考にする
  }
}
```
