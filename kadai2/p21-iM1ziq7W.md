...
**ユークリッドの互除法**とは２つの自然数の最大公約数（reatest common divisor）を効率的に求める方法である．2つの自然数 $a, b (a \geq b)$ について、$a$の$b$による剰余を $r$ とすると、$a$と$b$ の最大公約数は$b$と$r$との最大公約数に等しいという性質が成り立つ．この性質を利用して、$b$を$r$で割った剰余、 除数 $r$ をその剰余で割った剰余、と剰余を求める計算を逐次繰り返す．剰余が$0$になった時の除数が$a$と$b$との最大公約数となる．
...
<p align="center">
  <img src="prob2-1-1.jpg" width="400">
...
<p align="center">
  <img src="prob2-1-2.jpg" width="700">
...
- 以下のプログラムを必ず用いて,プログラムの`???`部分とコメントの`???`部分を適切に補うことにより,ユークリッドの互除法を実装すること．    
...
---
- まず，上記プログラムを書きうつす．
- ユークリッドの互除法のアルゴリズムの説明やコメントを参考にプログラムの`???`部分とコメントの`???`部分を埋める．
