...
- `int`型配列の先頭アドレスを保持するためのポインタ変数と，配列の長さを保持するための`int`型変数を持つ構造体として，`struct Vector`を定義すること．
```
struct Vector {
  ...
};
...
- 引数として配列の長さを取り，動的メモリ確保して`struct Vector`変数を返す`Vector_init`関数，および`struct Vector`変数を引数として取って確保された配列を解放する`Vector_free`関数を定義すること．前者では，メモリ確保に失敗した場合はエラー文を出力してプログラムを強制終了すること．
```
struct Vector Vector_init(int n){
  ...
...
- 引数として`struct Vector`変数と，配列の場所（インデックス）を示す`int`型変数を取り，そのインデックスの要素を返す`Vector_get_at`関数を定義せよ．インデックスが有効な範囲の外にある場合，エラー文を出力してプログラムを強制終了すること．
...
- 引数として`struct Vector`変数，配列インデックスを示す`int`型変数，配列に代入したい要素を示す`int`型変数を取り，そのインデックスに要素を代入する`Vector_set_at`関数を定義せよ．インデックスが有効な範囲の外にある場合，エラー文を出力してプログラムを強制終了すること．
