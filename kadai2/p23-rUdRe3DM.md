...
- 例えば、長さ6の配列の要素番号1~2に`5 7`,要素番号3~4に`6 8`がある場合,入力配列は`** 5 7 6 8 **`として与えられ(要素番号1~4以外は便宜上`**`で埋めてある),`merge`関数は次のように動く.

  - まず、要素番号1~4にある`5 7 6 8`からマージ用記憶領域（要素番号7~10）にソートされた配列`5 6 7 8`を作り出す.全体の長さ12の配列は`** 5 7 6 8 ** ** 5 6 7 8 **`となる.
...
- 要素間の比較`comp_`の呼び出し回数の合計も最後に標準出力する．出力の仕方は以下の通り.
  ```
  comp⊔=⊔13
  ```
