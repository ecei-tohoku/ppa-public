...
### ステップに分けてプログラミング
---
この問題では，「グローバル変数を用いた再帰+メモ化なしによる再帰関数の呼び出し回数のカウント」「再帰+メモ化ありによる編集距離の計算」「再帰+メモ化ありによる再帰関数の呼び出し回数の計算」の3ステップに分けてプログラミングしてもらう．ステップごとに"動作確認"に成功しなければ，"TAに提出"が出来ないようになっている．以下では，各ステップをさらに細分化して説明する．

---
### ステップ1/3　「グローバル変数を用いた再帰+メモ化なしによる再帰関数の呼び出し回数のカウント」
---
...
- 一度計算した$c_{i,j}$の値をメモする二次元配列`memo`を定義する.

- `ldmemo`関数の引数に`memo`を追加する.

- 一度計算した編集距離$c_{i,j}$の値を`memo`に格納させるように`ldmemo`関数を修正する.

- `ldmemo`関数を呼び出す際,すでに計算済みである場合は`ldmemo`関数を呼び出さずに,`memo`を呼び出すことにより計算コスト削減する. 

- 計算済みかどうかを判別するためにはどうしたらいいだろうか？編集距離$c_{i,j}$は必ず0以上の値をとるため,`memo`の値が0以上かどうかで計算済みかを判断すれば良い.そのためには、`memo`の初期化を工夫する必要がある.
