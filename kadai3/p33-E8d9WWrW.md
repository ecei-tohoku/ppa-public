...
#### 2-1. 準備
- ステップ1/3で作成したプログラムはそのままに、`ld`関数のコピーを作成し,ldmemo関数と名づける.

- 一度計算したものをメモする二次元配列`memo`を定義する.

- 一度計算したldmemo関数を二次元配列memoに収納する

- 次にldmemo関数を呼び出す際,すでに計算済みである場合はldmemo関数を呼び出さずに,memoを呼び出すことにより計算コスト削減する. 
