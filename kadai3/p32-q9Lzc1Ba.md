...
- 文字列$X$と$Y$の長さを$len_x$と$len_y$としたとき，`ld(char *X, int len_x-1, char *Y, int len_y-1)`，`ld(char *X, int len_x-1, char *Y, int len_y)`，`ld(char *X, int len_x, char *Y, int len_y-1)`の呼び出し回数を標準出力せよ．それぞれの呼び出し回数を$C_1$，$C_2$，$C_3$とすると，**標準出力の形式は「$C_1$⊔$C_2⊔$_3↩︎」**とする．
  - 例：$X$=a, $Y$=cの時
  ```
    1⊔1⊔1↩︎
  ```
