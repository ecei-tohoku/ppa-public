...
- (2) k近傍法により心臓病か否かの診断結果（`0`: 心臓病でない or `1`: 心臓病である）を返す関数 `knn_diag` を作成せよ．このとき，
    - (2a) 診断対象の患者を表す $n$ 次元の `double` 型の配列と，多数決に用いる近傍事例の数を引数として取れ（これ以外の引数も自由に追加してよい）．
...
- (2): (1) で読み込んだ過去の診断結果は `knn_diag` の引数として渡すのが望ましいが，グローバル変数として保持し，関数内で参照してもよいこととする．
