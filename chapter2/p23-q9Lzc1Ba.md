...
ステップ2では入力に応じて配列の大きさを決定する可変長配列と呼ばれる便利な機能を利用する．
可変超配列の定義は以下のように行われる．
```
int n;
scanf("%d", &n);
int array[n];
```
<span style="color:red">ここでは，int array[n]のように配列を定義する前にnの値を必要十分な値に初期化する必要がある．</span>
nの値を初期化する前に配列を定義する場合，予測できない動作の原因となる．
またnが大きすぎる場合もメモリが確保できず，また確保されなかったことも分からないため，プログラムが予期せぬ動作をする恐れがある．
