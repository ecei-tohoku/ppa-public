...
- 以下のコードにはコンパイルエラーが複数含まれていて、コンパイルが失敗する．
このコードは自動採点システムの中で初期コードとして現れるように設定されている．
また、修正の途中で”初期コードのロード”ボタンを押すと初期コードに戻る．
<span style="color:red">ここでは、コードのコピー中のエラーを防ぐため、仕様にあるコードを自分でエディターに手入力することを避けたい</span>
  ```
  #include <stdio.h>
