...
ステップ1は，int b[8];のようにあらかじめ大きさを指定し配列を定義した．
ステップ2では入力に応じて配列の大きさを決定する可変長配列（Variable-Length Array; VLA）と呼ばれる便利な機能を利用する．
VLAの定義は以下のように行われる．
```
int n;
scanf("%d", &n);
int array[n];
```
<span style="color:red">ここでは，int array[n]のように配列を定義する前にnを適切な値に初期化する必要がある．
nを初期化する前に配列を定義する場合，予測できない動作の原因となる．</span>
nが大きすぎる場合はメモリが確保できず，また確保されなかったことも分からないため，プログラムが予期せぬ動作をする恐れがある．

この問題ではVLAの範囲外アクセスによるランタイムエラーのデバッグを行う．
...
- １等賞の番号で配列が初期化されている．
  ...
  int winningNumber[5] = {10,23,44,45,21};                  //１等賞の番号を記憶するための配列
  ```
- 入力される当選番号を保存するためVLAを用意する．色々な宝くじに対応するようにVLAが使われており，入力される当選番号の数に合わせて配列の長さが実行時に設定される．
また，バーコード番号を保存するためにも長さ12の配列を用意する．
- 入力に対してシステムが以下の処理をする
  - 入力者の年が10歳未満の場合以下のように出力され，プログラムが終了される．
    ```
...
7. <font color="red"><b>初期コードの出力に不要な空白が含まれている場合があるため，仕様や実行例をよく確認してください．</b></font>
...
- <font color="red"><b>本問題でもコメント記載と適切なインデント・行間は採点事項であることに注意．</b></font>初期コードで'???'としている箇所を適切なコメントで置き換えること．インデントと行間は適切に入れること．
...
1. 初期コードにはコンパイルエラーは含まれていないのでその”コンパイル／実行”あるいは”動作確認”を行い，エラーを確認する．
2. どの配列のどこが範囲外アクセスになっているのかを確認しデバッグする．
  - 一つのデバッグ手法としては，コードを部分ごとにコメントアウトしエラーを確認することである．
  - しかしながら，コメントアウトのためコンパイラーの最適化により実際にアクセスしないように実行される場合もある．
  - 別の手法としては，配列をアクセスするインデックスを出力して確認する方法もある．
  - 部分実行とインデックスの出力は、一緒に利用すると効率よくデバッグできる．
3.  その他にも論理エラーがあればデバッグする．
