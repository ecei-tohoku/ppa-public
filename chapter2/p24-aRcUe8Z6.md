...
標準入力からデータ数$N$，単純移動平均を計算する短期日数$M_{s}$と長期日数$M_{l}$，$N$個の観測値からなる時系列データ$x_1, x_2, ..., x_{N}$を読み込み，標準出力に日付$t$，観測値$x_t$, $Ms$日単純移動平均${\rm SMA}^{(Ms)}_t$，$Ml$日単純移動平均${\rm SMA}^{(Ml)}_t$，$Ms$日加重移動平均${\rm WMA}^{(Ms)}_t$，$Ml$日加重移動平均${\rm WMA}^{(Ml)}_t$を出力するプログラムを書け．
