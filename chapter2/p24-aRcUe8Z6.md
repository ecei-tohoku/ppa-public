...
ここでは，単純移動平均と加重移動平均を求めるプログラムを作成しよう．

---
## 問題
---

標準入力からデータ数$N$，単純移動平均を計算する日数$M$，$N$個の観測値からなる時系列データ$x_1, x_2, ..., x_{N}$を読み込み，標準出力に日付$t$，観測値$x_t$, $M$日単純移動平均${\rm SMA}^{(M)}_t$および$M$日加重移動平均${\rm WMA}^{(M)}_t$を出力するプログラムを書け．
...
- $N$，$M$を格納する変数には`int`型，$x_{i}$を格納する変数には<font color=red>倍精度浮動小数点</font>型である`double`型を用いること．単精度浮動小数点型`float`を用いてはいけない．浮動小数点型の詳細については，[こちら](#floating_point)を参照のこと．
- $M$は$1\le M\le 20$，$N$は$M\le N\le 100$であると仮定してよい．
- 出力は，$t$，$x_t$，${\rm SMA}^{(M)}_{t}$，${\rm WMA}^{(M)}_{t} ($t=M, M+1, ..., N$)を一行ずつ出力すること（$t=1, ..., M-1$は${\rm SMA}^{(M)}$, ${\rm WMA}^{(M)}$が存在しないので出力しない）：
  
  ```
  M x_{M} SMA^{(M)}_{M} WMA^{(M)}_{M}
  M+1 x_{M+1} SMA^{(M)}_{M+1} WMA^{(M)}_{M+1}
  ...
  N x_{N} SMA^{(M)}_{N} WMA^{(M)}_{N}
  ```

- $t$ ($t=M, M+1, ..., N$)は整数で出力し，$x_t$，${\rm SMA}^{(M)}_{t}$，${\rm WMA}^{(M)}_{t}$ ($t=M, M+1, ..., N$)は小数点以下2位までを出力すること．
- 各行の数値の間には空白文字を入れること．
- 各行の末尾には改行文字`\n`を付けること．
- 入力される時系列データは，可変長配列（Variable Length Array; VLA）を用いて，入力された`N`だけ要素を持つ配列を確保し，格納すること．（VLAの使用方法についてはp2-3の説明を参照．）
