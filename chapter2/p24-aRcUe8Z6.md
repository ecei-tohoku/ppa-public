...
- 出力は，$t$，$x_{t}$，
- [](${\rm SMA}^{(M_s)}_{t}$，${\rm SMA}^{(M_l)}_{t}$，${\rm WMA}^{(M_s)}_{t}$，${\rm WMA}^{(M_l)}_{t}$ $(t=1, 2, ..., N)$)を一行ずつ出力すること．日数$t$に対応する移動平均が存在しない場合は，`NA`と表示すること（NA: Not Applicable）：
...
- 式(1)，(2)の計算式のとおり，$t$ごとに和を求めて計算すること．特に，前の$t$で求めた和を，

$$ {\rm SMA}^{(M)}_{t}$ $= {\rm SMA}^{(M)}_{t-1}-??+?? $$

のように使い回して計算してはいけない（理由は[参考情報](#precision)で述べる）．
