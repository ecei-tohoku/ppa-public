...
標準入力からデータ数$N$, 単純移動平均を計算する短期日数$M_s$と長期日数$M_l$を読み込み, 標準出力に日付$t$, 配列インデックス$i=t-1$, $M_s$日単純移動平均${\rm SMA}^{(M_s)}_t$，$M_l$日単純移動平均${\rm SMA}^{(M_l)}_t$のそれぞれの計算に必要な配列インデックス, $i$, $i-1$, ..., $i-M+1$を出力するプログラムを書け.
...
- 入力は，データ数$N$と平均を取る短期日数$M_s$，長期日数$M_l$が順に並んでいるとする.
- $N$, $M_s$, $M_l$を格納する変数は`int`型とする.
- $M_s$は$1\leq M_s\leq 5$, $M_l$は$10 \leq M_l \leq 20$, $N$は$M_l \leq N \leq 100$であると仮定して良い.
- 出力は,  
日付$t$, インデックス$i$, ${\rm SMA}^{(M_s)}_t$の計算に必要な配列インデックス, ${\rm SMA}^{(M_l)}_t$の計算に必要な配列インデックスを1行ずつ出力すること.日数$t$に対応する移動平均が存在しない場合は, `NA`と表示させること(NA: Not Applicable).
- 出力形式は, 以下のようにすること:  
```
day:t, index:i, short:短期日数の移動平均の計算に必要な配列インデックス, long:長期日数の移動平均の計算に必要な配列インデックス
```
- 移動平均の計算に必要なインデックスは、$i$, $i - 1$, ..., $i - M + 1$の順で出力し、インデックスの間には空白を加えること.
- 各行の末尾には改行文字`\n`をつけること.
