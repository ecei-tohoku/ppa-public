...
残念なことに，C 言語のコンパイルに必要な gcc を含む開発者向けのツールがデフォルトではインストールされていません．
この節では，それらをインストールする手順を説明します．

と言っても，OS X で開発者向けのツールをインストールする方法はとても簡単です（Windows と違って！）．
まず，ターミナルを起動します（ターミナルは Finder を開いて「アプリケーション」→「ユーティリティ」→「ターミナル」で起動できます）．
そして，次のコマンドを入力してください．
...
（ここで失敗する場合は，App Store で Xcode をインストールしてから再度実行してみてください）．
...
試しにターミナルで `emacs` と入力してみましょう．
演習室の計算機では Emacs の新しいウィンドウが開かれると思いますが，OS X のデフォルトではターミナル上に表示されるでしょう．
見た目やバージョンは違いますが，中身は同じ Emacs です．
