...
なお，このシステムは複数の受講生が同時に利用していますので，同時にソースコードが提出された場合は，提出順に採点されます．
...
テストに失敗した場合は，その失敗したテスト事例を使い，端末上でプログラムをデバッグして下さい．

![テスト結果](codexam_result.png "テスト結果")

テストに成功すると，ホーム画面で以下のように「レポート」へのリンクが表示されます．
このリンク先の画面を印刷し，レポートとしてください．

![レポートへのリンク](codexam_report.png "レポートへのリンク")

## 演習室以外で課題を進めるには

セキュリティ上の問題から，自動採点システムへのアクセスは学内のみに限定しています．
