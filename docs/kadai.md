...
0. [自動採点システム](http://localweb.ecei.tohoku.ac.jp/ppa/)にログインする．
1. 取り組む課題のページを開く
2. 同じ課題のエディターを開く
3. プログラムを入力・編集する．
4. プログラムをコンパイルする．
   * コンパイルエラーが出た場合は，プログラムをデバッグしてプログラムがうまく動かない原因を特定し．3に戻る
5. プログラムを提出(実行)する．
   * テストに失敗した場合は，プログラムをデバッグしてプログラムがうまく動かない原因を特定し，3に戻る．
6. TAに自動採点システムを通過したことを報告し，確認をしてもらう．
...
本演習では，ブラウザ上で動くエディタを使い，ソースコードを入力します．
[自動採点システム](http://localweb.ecei.tohoku.ac.jp/ppa/)にログインし，課題1-0のエディタを開く．
...
通常は，ターミナルなどで以下のコマンドを入れます．
```
$ gcc -Wall sum.c
```

なお，`-Wall`はコンパイルエラーにはならないものの，ソースコードに問題がありそうな場合に警告を表示するためのオプションです．
予期せぬバグを防止するため，**このオプションは必ず付けてください**．

本演習においては，コンパイルボタンを押します．
...
プログラムの動作確認・デバッグが完了したら，自動採点システム（[http://localweb.ecei.tohoku.ac.jp/ppa/](http://localweb.ecei.tohoku.ac.jp/ppa/)）に投稿します．
...
セキュリティ上の問題から，自動採点システム（[http://localweb.ecei.tohoku.ac.jp/ppa/](http://localweb.ecei.tohoku.ac.jp/ppa/)）は学内からのアクセスのみ許可しています．
