...
- `fgets`は，一行から最大`N-1`個<font color="red">（N個ではない！）</font>の文字を読み込み，配列strに格納する関数である（`stdin`で，標準入力から読むことを指定している）．一行に`N`個以上の文字がある場合，先頭から`N-1`個の文字が読み込まれる．一行に`N`個未満しか文字がない場合は，それらの文字全てが格納される．
- C言語では，文字列の末尾に特殊な文字コードである`\0`（NULL文字，ヌル文字）を挿入することが約束事になっており，`\0`によって文字列の末尾であることが判別できるようにしている．例えば，`prinft`関数では，文字列を出力するとき`\0`によって文字列の終わりを判断するので，終端に`\0`がない文字列を出力しようとするとエラーの原因となる．
...
#### 1-4. 入力された整数の各桁のASCIIコードと，'0'を引いた数値を出力
