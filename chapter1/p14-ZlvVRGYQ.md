...
- C言語では，文字列の末尾に特殊な文字コードである`\0`（NULL文字，ヌル文字）を挿入することが約束事になっており，`\0`によって文字列の末尾であることが判別できるようにしている．例えば，`prinft`関数では，文字列を出力するときヌル文字によって文字列の終わりを判断するので，ヌル文字がない文字列を出力しようとするとエラーの原因となる．
...
- 注意点として，自動採点システムでは，全ての入力の最後に改行コード`\n`が付加された形で入力が読み込まれる．例えば自動採点システムで`abc`を入力として「コンパイル/実行」を行うと，実際には`abc\n`が入力される．
