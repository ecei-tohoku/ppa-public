...
- C言語では，文字列の末尾に特殊な文字コードである`\0`（NULL文字，ヌル文字）を挿入することが約束事になっており，`\0`によって文字列の末尾であることが判別できるようにしている．例えば，prinft関数では，文字列を出力するときヌル文字によって文字列の終わりを判断するので，ヌル文字がない文字列を出力しようとするとエラーの原因となる．
