...
- C言語では，文字列の末尾に特殊な文字コードである`\0`（NULL文字，ヌル文字）を挿入することが約束事になっており，`\0`によって文字列の末尾であることが判別できるようにしている(例えば，prinftで文字列を出力するとき，ヌル文字によって文字列の終わりを判断するので，ヌル文字がない文字列を読み込むとエラーが起きる)．`fgets`には読み込んだ文字列の最後に`\0`を追加する機能が備わっており，最大`N-1`個しか文字を読み込めないのはこのためである．例えば，`fgets`で`"abc"`を読み込んだ場合に，実際のメモリ上には`abc\0`の4文字分(`\0`で一文字という計算)が記録されている．よって，`\0`の場所がわかれば桁数（文字列長）がわかる．コード例：
    ```
    for(i=0;i<N;++i){
      if(str[i] == '\0'){
        break;
