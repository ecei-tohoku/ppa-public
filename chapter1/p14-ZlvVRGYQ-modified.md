...
1-4. 入力された整数のASCIIコードと，'0'を引いた数値を出力
...
- 複雑な場合分けを避けるため，文字列は必ず整数であると仮定する．また，テストしやすいように60桁ではなく，4桁だけ処理をする．
...
-4桁より桁数が多いかどうかをどう判別すればよいか？fgetsでちょうど4桁読むだけでは判別ができない．簡単な例として以下のコードを実行し，4桁より桁数が多いかを判別できるか，入力を$123$，$1234$，$12345$と変えていって確かめてみよう．その次に，`N = 5;`を`N = 6;`に変えると何が起こるか，確かめてみよう．そうすることで，何桁読み込めばよいか，何文字分の文字配列を確保するべきかが分かるはずである．
...
- 入力された文字列の一桁ずつに対し，以下の３つを空白1文字で区切って出力せよ：
  1. 文字のまま出力
  2. ASCIIコードの整数として出力
  3. '1'を引いた結果を整数として出力
- なお，文字変数`c`をASCIIコードの整数として出力するための書式は，`printf("%d\n", c);`である．
