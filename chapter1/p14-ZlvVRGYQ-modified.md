...
- このステップでは，入力される文字列は必ず整数であると仮定する．また，テストしやすいように60桁ではなく，4桁だけ。。。

#### 1-4. 入力された整数のASCIIコードと，'0'を引いた数値を出力

- ASCIIコードに慣れるために，入力された文字を(1)文字として出力，(2)ASCIIコードの整数として出力，(3)文字同士の引き算を整数として出力．文字`c`をASCIIコードの整数として出力するための書式は`printf("%d\n", c);`である．
