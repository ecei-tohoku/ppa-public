...
これでは分かりづらいので、例として、`for`を用いて繰り返し足し算を行うコードを考えよう。
...
ループ変数iは繰り返し回数をカウントする変数である。
上のコードではi=0から出発して、iを一つずつ増やし、iの値が4より小さければ（つまり、i=3まで）xに1を足していき、
iの値が4になれば繰り返し処理を中止するプログラムである。
...
コードを書くのに慣れてくれば、足し算を簡略化して次のように書くことも可能である。
