...
- $i=4$は条件$i<4$を満たさないので、`for`文を終了、最終的な値は$x=4$
...
コードを少し修正して、`for`文で使われているループ変数$i$の値を$x$に足していくことも可能である。
...
- $i=4$は条件$i<4$を満たさないので、`for`文を終了、最終的な値は$x=6$
...
この場合、変化式が指定されていないため、$i$の値は常に$0$であり、条件式$i<4$が常に真となるため、プログラムは終了しない（自動採点システムで実行するとtime outとなり強制終了される）。
...
- $i=0$は条件$i<4$を満たすので、$x$に$i=0$を足して$x=0$、$i$に$1$を足して$i=1$
- $i=1$は条件$i<4$を満たすので、$x$に$i=1$を足して$x=1$、$i$に$1$を足して$i=2$
- $i=2$は条件$i<4$を満たすので、$x$に$i=2$を足して$x=3$、さらに`if`文の条件$i=2$を満たすので、`break`文により`for`文を強制終了、最終的な値は$x=3$
