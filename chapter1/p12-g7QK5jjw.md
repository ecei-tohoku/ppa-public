...
- i=3は条件i<4を満たすので、xに1=3を足してx=6、iに1を足してi=4
- i=4は条件i<4を満たさないので、`for`文を終了、最終的なxの値はx=6

注意点として、`for`文ではプログラミングのバグにより無限ループが発生することがしばしばある。
...
この例は少し極端だが、初期化式、条件式、変化式が適切に指定しないために無限ループが発生してしまう、というのはプログラミングにおいて頻繁に遭遇することである。
...
繰り返し処理を行っているとき、繰り返し処理を途中でスキップして、次の繰り返し処理を行いたい場合が
このような状況で役に立つのが`continue`文である。
