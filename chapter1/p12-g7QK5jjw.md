...
この場合、xは常に1以上なので、条件式x>0が常に真であり、プログラムは終了しない（自動採点システムで実行するとtime outとなり強制終了される）。
この例はあまりに極端だが、本来使うべきダミー変数iとは異なる変数を条件式と変化式に用いてしまったために無限ループが発生してしまう、ということはしばしば起きることである。
