...
### 配列の使い方
---
- ある型の変数を１個以上まとめて扱うことができるのが配列である（数学でいうと`1`行×`n`列の行列のイメージ）．
- 例えば`int`型の変数10個の配列は，`int a[10];`と定義する．
- 通常の変数と同じく，配列内の値にアクセスする（値を入れる，値を取り出す）方法は以下のとおりになる:
    ```
    int a[10];
    int x;
...
- 配列のインデックス（鍵括弧内の数字）は`1`からではなく，`0`で始まる．そのため，サイズ`n`の配列は`a[0]`から`a[n-1]`までしかアクセスしてはいけない．`a[n]`のようにアクセスしないこと．
- よくあるパターンは，以下のような`for`文を使った配列操作である（`1`から`19`までの奇数を配列に代入）:
   ```
    int a[10];
