...
int main() {
  ...
  char buf[vmin_(len_x, len_y)];   // マッチングした文字列を保存する配列bufの宣言

  int cnt = find_matching(data_x, len_x, data_y, len_y, buf);  // 2つの文字列の中でマッチした文字数を返す関数を実行
  sort_buf(buf, cnt);                                          // マッチした文字列をソートする関数を実行
