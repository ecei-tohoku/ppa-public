...
＋ 以下のプログラムを必ず用いてプログラムを完成せること．
  +  <font color="red">`???`</font> の部分に必ずコメントを入れること．

```
#include <stdio.h>
#include <stdlib.h>

int vmax_(int x, int y) { return x > y ? x : y; }                  // <font color="red">???</font>
int vmax3_(int a, int b, int c) { return vmax_(a, vmax_(b, c)); }  // <font color="red">???</font>
int vmin_(int x, int y) { return x < y ? x : y; }                  // <font color="red">???</font>
int vmin3_(int a, int b, int c) { return vmin_(a, vmin_(b, c)); }  // <font color="red">???</font>
...
char* read_string_(  // 文字列用のメモリ確保と、標準入力からの文字列読み込みを行う関数
  ...
  if (data[N - 1] != '\0') {  // <font color="red">???</font>
