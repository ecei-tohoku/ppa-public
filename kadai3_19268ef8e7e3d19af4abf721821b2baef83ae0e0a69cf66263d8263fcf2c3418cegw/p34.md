...
+ 編集距離を求めるために、既に用意されている関数`get_dp_table_`を用いて編集距離および動的計画法のテーブルを取得すること(この関数を問題3-1, 3-2, 3-3で使用することは禁止する).
  ...
  + 例：
    ...
    void get_dp_table_(char *X, int m, char *Y, int n, 
                       int dpt[m＋1]][n+1]);
    ```
  + 使用する際には,`#include "ppa_extra_h/p3_header.h"`を宣言すること.
...
+ 動的計画法用の二次元配列テーブル`dpt`を表示すること．
  ...
  + 実行例や以下の例を参考にし，文字列$X$を左端（縦）に，文字列$Y$を先頭（横）に出力し，$c_{i,j} (i \in \{0, 1, ..., m\}, j \in \{0, 1, ..., n\})$のすべての値をスペース区切りの行列形式で出力せよ．行末に余分なスペースを出力しないように注意せよ．
    ...
    + 1行目は文字列$Y$の文字を2文字分のスペース区切りで出力せよ．行頭に3文字分のスペースを挿入せよ．
...
int main() {
  ...
  char ops[???], sep_x[???], seq_y[???];
  get_dp_table_(data_x, len_x, data_y, len_y, dpt);  // ???
  show_matrix(data_x, lenx_x, data_y, len_y);
