...
+ 注意点として, <font color="red">$c_{3,4} \rightarrow c_{2,3} \rightarrow c_{2,2}$</font> ではなく, $c_{3,4} \rightarrow c_{3,3} \rightarrow c_{2,2}$ を通る経路でも編集距離は同じである.しかし,同じ編集距離を与える場合は,バックトレースの出発点から見てコピー/置換を優先することにしたので,$c_{3,4} \rightarrow c_{3,3}$ ではなく, $c_{3,4} \rightarrow c_{2,3}$を通る経路が我々の求める最短経路である.
