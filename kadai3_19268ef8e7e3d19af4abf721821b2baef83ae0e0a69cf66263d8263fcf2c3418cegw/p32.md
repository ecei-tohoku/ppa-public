...
問題3-1で実装したプログラムにメモ化を導入し，メモ化ありの場合とメモ化なしの場合の再帰関数の呼び出し回数を求めよ.
...
### 仕様
---
+ 課題3-1に記載の仕様．ただし，「以下のプログラムを必ず用いてプログラムを完成させること」の部分は除く．
...
+ `memo`の大きさは文字列の長さに合わせて必要十分な大きさとすること．
...
+ ステップ3：ステップ2が正しく実装できると，文字列$X$の長さを$l_x$，文字列$Y$の長さを$l_y$とすると，再帰関数の呼び出し回数は$(l_x+1) \times (l_y+1)$になる．これを$l_x \times l_y$に減らすにはどうしたら良いかを考えよ．
