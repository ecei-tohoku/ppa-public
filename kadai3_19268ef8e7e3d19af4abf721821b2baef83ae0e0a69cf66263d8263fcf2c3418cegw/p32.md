...
+ 2次元配列の確保と開放には，`func_mallocation_2d_`関数と`func_free_2d_`をそれぞれ用いよ．
  + `func_mallocation_2d_`関数は，整数$l_1$と整数$l_2$を引数に取り，整数型の2次元配列を返す関数である．例えば，以下のように書くと，$l_1$行$l_2$列の2次元配列arrayが確保出来る．
  ```	
  int** array = func_mallocation_2d_($l_1$, $l_2$);
  ```
  + `func_free_2d_`関数は，2次元配列arrayと整数$l_1$を引数に取るvoid型の関数である．例えば，以下のように書くと，2次元配列arrayの開放を行う．
  ```	
  func_free_2d_(array, $l_1$);
  ```
  + これらの関数は，`ppa_extra_h/p3_header.h`においてある．`#include "ppa_extra_h/p3_header.h"`という宣言をする事で使用可能である．これらの関数のソースコードは，下の参考に書いたので，興味ある者は参照せよ．
...
## 参考
---
`func_mallocation_2d_`関数と`func_free_2d_`のソースコードは，それぞれ以下の通りである．
