...
+ 最小の経路は, $c_{5,6} \rightarrow c_{4,5} \rightarrow c_{3,4} \rightarrow \rightarrow c_{2,3} \rightarrow c_{2,2} \rightarrow c_{1,1} \rightarrow c_{0,0} $である.
+ 注意点として, $c_{2,3}$ ではなく, $c_{3,3}$ を通る経路でも編集距離は同じである.しかし,同じ編集距離を与える場合は,バックトレースの出発点から見てコピー/置換を優先することにしたので,$c_{3,4} \rightarrow c_{3,3}$ ではなく, $c_{3,4} \rightarrow c_{2,3}$を通る経路が我々の求める最短経路である.
