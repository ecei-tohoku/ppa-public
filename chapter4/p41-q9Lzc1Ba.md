...
このとき，先頭の文字から順番にX[0]とZ[0]の文字を比較，X[1]とZ[1]の文字を比較...としていけば編集距離が計算できる.

**重要** 削除（delete）と挿入（insert）の操作を無視しているので，ステップ1/3とステップ2/3で計算する編集距離は，正しい編集距離ではないことに注意せよ.例として，eatとateの正しい編集距離は3であるが，ステップ1/3とステップ2/3で計算する編集距離では

#### 1-1. `delta`関数の実装
- 関数の引数を記入し，関数の中身を実装する．

#### 1-2. `for`ループを用いて編集距離を計算する
- `delta`関数を用いて，二つの文字の長さが同じ場合に対して編集距離を計算する.
