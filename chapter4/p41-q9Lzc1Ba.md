...
これは，二つの接頭辞$X_{i-1} = \langle x_1, x_2, ..., x_{i-1} \rangle$と$Y_j = \langle y_1, y_2, ..., y_j \rangle$の編集距離$c_{i-1, j}$の値が既知のとき，新しい接頭辞$X_{i} = \langle x_1, x_2, ...,x_{i-1} x_{i} \rangle$に対して$x_{i}$を削除（delete）すれば元の$X_{i-1}$が得られるため，$c_{i-1, j} + 1$が成立するからである．
