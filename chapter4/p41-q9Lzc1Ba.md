...
まずは問題を簡単にするために，ステップ1/3では削除（delete）と挿入（insert）の操作は忘れて，コピー（copy）と置換（replace）のみを編集操作とし，さらに，二つの文字列$X$と$Z$の長さが同じ場合を考えよう.
このとき，先頭の文字から順番にX[0]とZ[0]の文字を比較，X[1]とZ[1]の文字を比較...としていけば編集距離が計算できる.

**重要** 削除（delete）と挿入（insert）の操作を無視しているので，ステップ1/3とステップ2/3で計算する編集距離は，正しい編集距離ではないことに注意せよ.
