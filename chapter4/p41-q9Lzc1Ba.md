...
## ステップに分けてプログラミング
---
この問題では，「`delta`関数を実装し，二つの文字の長さが同じ場合に対して，`for`ループを用いて編集距離を計算する」「二つの文字の長さが同じ場合に対して，`ld`関数の再帰部分を実装し，編集距離を計算する」「一般の場合に対して，`ld`関数を完成させ，編集距離を計算する」の3ステップに分けてプログラミングしてもらう．ステップごとに”動作確認”に成功しなければ，”TAに提出”が出来ないようになっている．以下では，各ステップをさらに細分化して説明する．

---
## ステップ1/3　「`delta`関数を実装し，二つの文字の長さが同じ場合に`for`ループを用いて編集距離を計算する」
---
ここでは，二つの文字の長さが同じ場合に限定し，再帰を用いずに`for`ループを用いて編集距離を計算しよう.
二つの文字の長さが同じ場合は先頭の文字から順番にX[0]とY[0]の文字を比較，X[1]とY[1]の文字を比較，...，としていけば編集距離が計算できる.

#### 1-1. `delta`関数の実装
- 関数の引数を記入し，関数の中身を実装する．

#### 1-2. `for`ループを用いて編集距離を計算する
- `delta`関数を用いて，二つの文字の長さが同じ場合に対して編集距離を計算する.ここでは`main`関数の中で編集距離を計算してよい.
...
- また，二つの文字の長さが同じ場合のみを考えているので$m$と$n$は常に同じ値であるが，ステップ3/3のために，`ld`関数は`ld(X,m,Y,n)`として定義すること．
...
#### 2-2. 漸化式の「$i=0$又は$j=0$」の部分を`ld`関数に実装

#### 2-3. 漸化式のコピー（copy）と置換（replace）の部分を`ld`関数に実装
...
#### 3-1. 二つの文字の長さが異なる場合も考慮し，削除（delete）と挿入（insert）の操作も漸化式に実装
