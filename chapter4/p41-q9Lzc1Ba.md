...
## ステップに分けてプログラミング
---
この問題では，「`delta`関数を実装し，二つの文字の長さが同じ場合に対して，`for`ループを用いて(正しくない)編集距離を計算する」「同じ処理を再帰を用いて実装し，(正しくない)編集距離を計算する」「`ld`関数を完成させ，(正しい)編集距離を計算する」の3ステップに分けてプログラミングしてもらう．ステップごとに”動作確認”に成功しなければ，”TAに提出”が出来ないようになっている．以下では，各ステップをさらに細分化して説明する．
...
このとき，先頭の文字から順番に$x_1$と$y_1$の文字を比較，$x_2$と$y_2$の文字を比較...としていけば編集距離が計算できる.

**重要** 削除（delete）と挿入（insert）の操作を無視しているので，ステップ1/3とステップ2/3で計算する編集距離は正しい編集距離ではないことに注意せよ.例として，eatとateの正しい編集距離は2であるが，ステップ1/3とステップ2/3では先頭の文字から$x_1$と$y_1$，$x_2$と$y_2$，$x_3$と$y_3$の全ての文字が異なるので編集距離は3となる.
...
#### 1-3. 標準出力の部分を実装する
- 仕様の標準出力の部分と以下の「ステップ1/3の実行例」を参照せよ．
...
---
## ステップ2/3　「二つの文字の長さが同じ場合に対して，`ld`関数の再帰部分を実装し，編集距離を計算する」
---

- 問題1-2を参考にして，`delta`関数を用いて`ld`関数の再帰部分を実装せよ．`for`ループは用いないこと．

- ただし，入力される二つ文字の長さが同じ場合のみを考えているので，編集距離を計算する際はコピー（copy）と置換（replace）のみを考えればよく，削除（delete）と挿入（insert）を考える必要はない．

- ステップ2/3では，`ld`関数の再帰を正しく実装出来たかをチェックするために，`ld(X,m,Y,n)`関数を呼び出した直後に次の表示を行うこと．
...
---
## ステップ3/3　「`ld`関数を完成させ，編集距離を計算する」
---
ここでは，`ld`関数を完成させ，二つの文字の長さが異なる場合に対しても編集距離を計算する．

#### 3-1. 二つの文字の長さが異なる場合も考慮し，漸化式の削除（delete）と挿入（insert）の処理も実装
