...
+ 問題4-1に提示してある関数を`ppa_extra_h/p4_header.h`というファイルにおいてある．`#include "ppa_extra_h/p4_header.h"`という宣言を通して含め，以下の関数を必要に応じて用いること．
  + `vmax_`
  + `vmax3_`
  + `vmin_`
  + `vmin3_`
...
+ 2次元配列の確保と開放には，`ppa_extra_h/p4_header.h`に実装してある`malloc_2d_`関数と`free_2d_`関数をそれぞれ用いよ．
...
### ステップに分けてプログラミング
---
この問題では，「グローバル変数を用いた再帰関数の呼び出し回数のカウント」「メモ化の実装」「メモ化を実装した場合のグローバル変数を用いた再帰関数の呼び出し回数のカウント」「インライン展開による呼び出し回数の削減」の4ステップに分けてプログラミングしてもらう．ステップごとに"動作確認"に成功しなければ，"TAに提出"が出来ないようになっている．以下では，各ステップをさらに細分化して説明する．

---
### ステップ1/4　「グローバル変数を用いた再帰関数の呼び出し回数のカウント」
---
...
#### 2-1. 準備
- ステップ1/4で作成したプログラムはそのままに，`ld`関数のコピーを作成し,`ldmemo`関数と名づける. ただし，引数に二次元配列も追加すること. 以下では,`ldmemo`関数にメモ化の機能を実装させるが,`ldmemo`関数の呼び出し回数を一意に定めるために,必ず下記の手順に従いメモ化を実装すること.
...
#### 2-3. メモ化の実装
- `ldmemo(X,i,Y,j)`で編集距離$c_{i,j}$の値の計算を始める前に，計算済みの値が`memo`に格納されていないか確認する．
  + もし格納されている場合はその値を返す．
  + もし格納されてない場合は編集距離$c_{i,j}$の値を再帰的に`ldmemo`関数を呼び出して計算し，`WriteMemo`関数を用いて`memo[i][j]`に格納してから返す.
    ```c
    int ldmemo( ??? ) {
...
- `memo`を初期化する場合は`WriteMemo`関数を使わず直接`memo[i][j]`に初期値を代入する．


#### 2-4. `ldmemo`関数による編集距離の値を確認
- ここでは`memo`を用いて`ldmemo`関数が編集距離を正しく計算できていることを確認する(`ldmemo`関数の呼び出し回数はステップ3/4で計算する).ステップ1/4の出力を改良して,以下の書式で表示すること.
...
---
### ステップ3/4　「メモ化を実装した場合のグローバル変数を用いた再帰関数の呼び出し回数のカウント」
---
このステップではメモ化により,`ldmemo`関数の呼び出し回数が削減されていることを確認する.

#### 3-1. グローバル変数の定義
- ステップ2/4で作成したプログラムはそのままに,`main`関数の外で整数型のグローバル変数`countm`，`readCountm`, `writeCountm`を定義する.
...
- `countm`の値は，必ず`ldmemo`関数の冒頭で更新せよ．
  ```c
  int ldmemo( ??? ) {
...
---
### ステップ4/4　「インライン展開による呼び出し回数の削減」
---
ステップ3/4までの実装では，`memo`へのアクセスを`ldmemo`関数の冒頭と末尾で行っていたが，他のアプローチとして，実際の処理の中で必要になった時点で`memo`の値を確認する方法も考えられる．
すなわち，`ldmemo`関数の中で`ldmemo`を再帰的に呼び出す箇所で，`memo`の確認と値の取得もしくは計算（再帰呼び出し）をその都度記述する方式である．
このように，関数を呼び出す代わりにその関数の処理を直接展開して書いたり，あるいは同様の最適化を行うコンパイラーの機能のことを**インライン展開**と呼ぶ．
ステップ4/4では，インライン展開により呼び出し回数が削減できることを確かめる．

#### 4-1. インライン展開の実装

- ステップ2/4で実装した，`ldmemo`関数冒頭で`memo`を確認する処理を削除する（次に同様の処理を`ldmemo`関数内に挿入するので，一旦コメントアウトしておくとよい）．

- 代わりに`ldmemo`を再帰的に呼び出している箇所で，`memo`の確認と値の取得もしくは再帰呼び出しを行う処理を記述する．

#### 4-2. グローバル変数の値を確認

- ステップ3/4と同じフォーマットで結果を出力し，呼び出し回数の変化を目視で確認せよ．
