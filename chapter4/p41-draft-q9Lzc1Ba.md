...
2つの文字列の類似性が測定できると役に立つ場合がある．例えば，

```text
AAAAAGATTAGCATC
```

というDNA塩基配列に対して，次の配列のうちより似ている方はどちらだろうか．
...
前者の方が似ているように感じるが，これを定量的に評価するにはどうすればいいだろうか．
本課題では，2つの文字列の類似性を測定する方法として，**編集距離（edit distance, Levenshtein distance）**に取り組む．

---
### 編集距離
---

2つの文字列 $X$ と $Y$ が与えられたときに，文字列 $X$ を1文字ずつ編集して文字列 $Y$ に変換することを考える．ここで**編集操作**として，

+ **置換 (replace)**: $X$ の1文字を別の文字に置換する．
+ **削除 (delete)**: $X$ の1文字を削除する．
+ **挿入 (insert)**: 1文字を $X$ に挿入する．

の3つを導入する．例えば，$X = \text{AAAAAGATTAGCATC}$，$Y = \text{AAAAGATTAGCATCC}$ としたとき，次のような編集で文字列 $X$ を $Y$ に変換できる．
...
代わりに $Y = \text{AACAAGCTTATGCTA}$ であれば，
...
となる．ここで，`R`，`D`，`I`はそれぞれ置換，削除，挿入をした箇所を表す．編集操作の回数を数えると，前者は2回で後者は6回なので，前者の方がより似ている文字列のペアと言えそうである．
ただし，文字列 $X$ から $Y$ を得る編集の過程は複数存在する．例えば最初の例で，置換のみを用いて文字列 $Y$ を得るのであれば，
...
としてもよい．そこで編集距離を，**文字列 $X$ から文字列 $Y$ を得るための編集操作の最小回数**として定義する．

---
### 編集距離の定義
---

以上のことを形式的に定義しよう．入力として長さ $m$ の文字列 $X = \langle x_1, x_2, ..., x_m \rangle$ と長さ $n$ の文字列 $Y = \langle y_1, y_2, ..., y_n \rangle$ が与えられる．
次に空の文字列 $Z$ を用意し，文字列 $X$ の先頭から任意回の**編集操作**を逐次適用しながら，文字列 $Y$ に等しい文字列 $Z = \langle z_1, z_2, ..., z_n \rangle$ を得る手続きを考える．
また， $X,Z$ の編集操作の対象となる添字を変数 $i,j$ で表現し，初期状態を $i=j=1$ とする．すると，前述の手続きは，「 $Y$ と等しい $Z$ を得るまで (1) 編集操作の適用，(2) 操作対象の添字 $i,j$ の更新，を繰り返す」という手続きで表現できる．

手続きの表現のために，最初に挙げた3つに**コピー (copy)**を加えた4つを編集操作として定義する．

| 編集操作 | 説明 | $Z$ の操作 | 添字の更新 |
|:-|:-|:-:|:-:|
| **コピー (copy)** | $X$ の1文字を $Z$ にコピーする | 代入 $z_j \gets x_i$ | $i$ と $j$にそれぞれ $1$ を加える |
| **置換 (replace)** | $X$ の1文字を別の文字 $\alpha$ に置換する | 代入 $z_j \gets \alpha$ | $i$ と $j$ にそれぞれ $1$ を加える |
| **削除 (delete)** | $X$ の1文字を削除する | 何もしない | $i$ にのみ $1$ を加える |
| **挿入 (insert)** | 文字 $\alpha$ を $Z$に挿入する | 代入 $z_j \gets \alpha$ | $j$ にのみ $1$を加える |
{: .table .table-striped}

例として"algorithm"と"altruistic"という2つの文字列に対して，
...
という編集操作に対応する過程の例を示す．
ここで， $X$ の太字と $Z$ のアスタリスク `*` は，操作対象の添字 $i,j$ が指し示す位置を表す．$Z$ の アンダーバー `_` は，空文字を表す．
...
さて，ここで行われた編集操作はそれぞれ，コピーが4回・置換が2回・削除が3回・挿入が4回だった．
ここでは単純な文字列の違いに興味があるので，やはりコピー以外の3つの編集操作の回数を指標として使うのが良さそうである．
より一般的には，各編集操作 $a$ に対してコスト ${\rm cost}(a)$ を与えると，上記の編集過程で要するコストは次のように計算できる．
...
編集距離を効率良く求めるにはどうしたらいいだろうか．まず，**接頭辞 (prefix)** という概念を導入する．接頭辞は，ある文字列の先頭から何文字かを抜き出した部分文字列である．形式的に定義するのであれば，

+ ある文字列 $X = \langle x_1, x_2, ..., x_m \rangle$ の長さ $p \in \lbrace0, 1, ..., m\rbrace$ の**接頭辞**を $X_p = \langle x_1, x_2, ..., x_p \rangle$ と定義する．

例えば， $X = \langle A, T, C, G \rangle$ に対し， $X_2 = \langle A, T \rangle$ である．接頭辞は $p = 0$ の場合として空文字列 $X_0$ を含むことに注意せよ．
...
+ （$x_i = y_j$の場合のみ）$c_{i-1,j-1}$: $Y_{j-1}$に$x_i$を*コピー*する
+ （$x_i \neq y_j$の場合のみ）$c_{i-1,j-1} + 1$: $x_i$を$\alpha$に*置換*して，$Y_{j-1}$に追加する
+ $c_{i-1,j} + 1$: $x_i$を*削除*し，$Y_j$を得る
+ $c_{i,j-1} + 1$: 新しい文字$\alpha$を*挿入*し，$Y_j$を得る

以上のことから，**編集距離$c_{m,n}$を求める問題は，部分問題，すなわち編集距離$c_{m-1,n-1}$, $c_{m-1,n}$, $c_{m,n-1}$を求める問題に分解できる．**


例えば，$X = \langle a, b \rangle$ と $Y = \langle c, d\rangle$ に対する編集距離$c_{2,2}$は次のように求まる．$X$と$Y$の編集距離は$2$である．
<div align="center">
...
漸化式の成立についてもう少し詳しく説明しよう．

---

#### コピーもしくは置換

2つの接頭辞 $X_{i-1} = \langle x_1, x_2, ..., x_{i-1} \rangle$ と $Y_{j-1} = \langle y_1, y_2, ..., y_{j-1} \rangle$ の編集距離 $c_{i-1, j-1}$ の値が既知のとき，
接頭辞 $Z_{j-1}$ に文字を1つ追加して接頭辞 $Y_{j}$ を作るときのコストを計算する．

文字 $x_{i}$ と $y_{j}$ が等しければ，接頭辞 $Z_{j-1}$ に文字 $x_{i}$ をコピーして接頭辞 $Z_{j}$ を得ることができるので,

$$
\begin{align}
c_{i,j} &= c_{i-1, j-1} + {\rm cost}(\mbox{copy}) \\
        &= c_{i-1, j-1} + 0
\end{align}
$$

が成立する．文字 $x_{i}$ と $y_{j}$ が等しくなければ，文字 $x_{i}$ を文字 $y_{j}$ に置換して接頭辞 $Z_{j-1}$ に追加すれば接頭辞 $Z_{j}$ を得ることができるので,

$$
\begin{align}
c_{i,j} &= c_{i-1, j-1} + {\rm cost}(\mbox{replace}) \\
        &= c_{i-1, j-1} + 1
\end{align}
$$
...
接頭辞 $X_{i}$ の終端の文字 $x_{i}$ を削除すれば $X_{i-1}$ が得られるから，削除の操作を行うときの漸化式は，

$$
\begin{align}
c_{i,j} &= c_{i-1, j} + {\rm cost}(\mbox{delete}) \\
        &= c_{i-1, j} + 1
\end{align}
$$

と表現できる．[編集距離の定義](#編集距離の定義)で説明したように，削除はインデックス$i$を1つ進める（$X$の文字を1文字無視する）のと対応していることに注意せよ．

---

#### 挿入

削除と同様に，接頭辞 $Z_{j-1}$ の終端に文字 $y_{j}$ を挿入すれば $Z_{j}$ が得られるので，

$$
\begin{align}
c_{i,j} &= c_{i, j-1} + {\rm cost}(\mbox{insert}) \\
        &= c_{i, j-1} + 1
\end{align}
$$
...
最終的な編集距離 $c_{i, j}$ を得るには，これら3つの編集操作を試行して，そのうちコストが最小の操作を選べばよいから，$i\neq0$ かつ $j\neq0$ ならば漸化式
...
+ コピーもしくは置換・削除・挿入の3つのコストから最小値を求める処理では，必ず3種類の値を求めて評価すること．例えば何らかの条件によっていずれかのコストの計算を省略する実装にはしないこと．
